...
class Toolset extends Post {
	...
	public function get_description() {
		return esc_html__( 'This shortcode is useful for retrieving Toolset Fields.', 'dynamic-shortcodes' );
	}
	...
	public function get_first_steps_examples() {
		...
		$format = esc_html__( 'To use the syntax, you should write %1$s where %2$s represents the name of a specific Toolset field you wish to select from the current post. If you want to select a specific post, you can use %3$s followed by the post\'s identification number. For example, combining field name with post ID, writing %4$s will retrieve the fields related to the post with ID %5$d.', 'dynamic-shortcodes' );
