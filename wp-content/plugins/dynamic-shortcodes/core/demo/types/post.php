...
class Post extends BaseDemo {
	...
	public function get_description() {
		...
		$message = esc_html__( 'This shortcode is useful for retrieving Post Fields, also called postmeta. In this example we show you all the custom fields available on the page %1$s. You may not recognise all fields as some are created by third-party plugins.', 'dynamic-shortcodes' );
	...
	public function get_first_steps_examples() {
		...
		$format = esc_html__( 'To use the syntax, you should write %1$s where %2$s represents the name of a specific field you wish to select from the current post. If you want to select a specific post, you can use %3$s followed by the post\'s identification number. For example, combining field name with post ID, writing %4$s will retrieve the fields related to the post with ID %5$d.', 'dynamic-shortcodes' );
	...
	public function generate_sample_id() {
		return [
			[
				'name' => [ BaseDemo::class, 'generate_post_id' ],
				'arg' => 'title',
				'keyarg' => 'ID',
