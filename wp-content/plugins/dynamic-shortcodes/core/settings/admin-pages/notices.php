...
class Notices {
	const DB_PREFIX = DYNAMIC_SHORTCODES_PREFIX . '_dismissed_';
	/**
	 * @var bool
	 */
	...
	public function action_admin_init() {
		...
		$dismiss_key = $_GET['dynamic_shortcodes_dismiss'] ?? false;
	...
	public static function render_notice( $message, $type, $dismiss_key = false ) {
		$classes      = "notice dynamic-shortcodes-generic-notice notice-$type";
		$dismiss_attr = '';
		if ( $dismiss_key ) {
			$classes      .= ' dynamic-shortcodes-dismissible-notice is-dismissible';
			$dismiss_url   = add_query_arg(
				array(
					'dynamic_shortcodes_dismiss' => $dismiss_key,
				),
				admin_url()
			);
			$dismiss_attr .= ' data-dismiss-url="' . esc_url( $dismiss_url ) . '"';
		}
		$icon_url          = DYNAMIC_SHORTCODES_URL . 'assets/media/dynamic-ooo.png';
		$product_name_long = DYNAMIC_SHORTCODES_PRODUCT_NAME_LONG;
		$html              = <<<EOD
<div class="$classes" $dismiss_attr>
<div class="img-responsive pull-left">
	<img class='dynamic-ooo-logo' src="$icon_url" title="$product_name_long">
</div>
